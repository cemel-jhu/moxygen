  ...
  case 'object':
    if (Array.isArray(element)) {
      element.forEach(function(value,
                               key) { s += toMarkdown(value, context); });
    } else {
      ...
      case 'parameteritem':
        ...
        break;
      case 'programlisting':
        s = '\n```' + module.exports.parserOptions.language + '\n';
        break;
      case 'orderedlist':
...
module.exports = {
  ...
  parseMembers : function(compound, props, membersdef) {
    ...
    if (membersdef) {
      membersdef.forEach(function(memberdef) {
        ...
        Object.keys(memberdef.$)
            .forEach(function(prop) { member[prop] = memberdef.$[prop]; });
  ...
  parseMember : function(member, section, memberdef) {
    ...
    case 'function':
      ...
      if (memberdef.templateparamlist) {
        m.push('template<');
        if (memberdef.templateparamlist.length > 0 &&
      ...
      if (module.exports.parserOptions.language != "python") {
      ...
      if (memberdef.param) {
        memberdef.param.forEach(function(param, argn) {
          if (module.exports.parserOptions.language != "python") {
            ...
            if (Q.length > 0 || (ptype.match(/\[/g) || []).length !=
                                    (ptype.match(/\]/g) || []).length) {
              Q += (Q.length > 0 ? ", " : "") + ptype;
              if ((Q.match(/\[/g) || []).length !=
                  (Q.match(/\]/g) || []).length) {
                return;
              ...
              ptype = toMarkdown(Q);
              Q = "";
            } else if ((ptype.match(/\[/g) || []).length == 0) {
              ptype = toMarkdown(param.type);
            }
            m = m.concat(argn == 0 ? [] : ', ');

            if (ptype.length > 0 && ptype[0] == "*") {
              m = m.concat(
      ...
      m = m.concat(')');
      if (module.exports.parserOptions.language == "python") {
        var def_type = memberdef.type
        if (def_type[0]._ != "def") m =
            m.concat(' → ', toMarkdown(memberdef.type));
      }
    ...
    case 'enum':
      ...
      m = m.concat(
          [ member.kind, ' ', markdown.refLink(member.name, member.refid) ]);
      break;
  ...
  assignToNamespace : function(compound, child) {
    if (compound.name != child.namespace)
      console.assert('namespace mismatch: ', compound.name,
                     '!=', child.namespace);
  ...
  assignNamespaceToGroup : function(compound, child) {
    ...
    Object.keys(child.compounds)
        .forEach(function(id) { delete compound.compounds[id]; });
  },
  ...
  extractPageSections : function(page, elements) {
    elements.forEach(function(element) {
      if (element['#name'] == 'sect1' || element['#name'] == 'sect2' ||
          element['#name'] == 'sect3') {
        var id = element.$.id;
        var member = {
  ...
  parseCompound : function(compound, compounddef) {
    ...
    Object.keys(compounddef.$)
        .forEach(function(prop) { compound[prop] = compounddef.$[prop]; });
    ...
    case 'group':
      ...
      if (compounddef.innerclass) {
        compounddef.innerclass.forEach(function(innerclassdef) {
          if (compound.kind == 'namespace') {
            // log.verbose('Assign ' + innerclassdef.$.refid + ' to namespace '
            // + compound.name);

            if (this.references[innerclassdef.$.refid])
              this.assignToNamespace(compound,
                                     this.references[innerclassdef.$.refid]);
          } else if (compound.kind == 'group') {
            // log.verbose('Assign ' + innerclassdef.$.refid + ' to group ' +
            // compound.name);
            if (this.references[innerclassdef.$.refid])
              this.assignClassToGroup(compound,
                                      this.references[innerclassdef.$.refid]);
          }
        }.bind(this));
      }
      ...
      if (compounddef.innernamespace) {
        ...
        compounddef.innernamespace.forEach(function(namespacedef) {
          if (compound.kind == 'group') {
            // log.verbose('Assign namespace ' + namespacedef.$.refid + ' to
            // group ' + compound.name);
            this.assignNamespaceToGroup(compound,
                                        this.references[namespacedef.$.refid]);
          }
        }.bind(this));
      }
      break;
  ...
  parseIndex : function(root, index, options) {
    index.forEach(function(element) {
      ...
      if (compound.kind !== 'file') { // && compound.kind !== 'file'
        log.verbose('Parsing ' +
                    path.join(options.directory, compound.refid + '.xml'));
        doxygen = fs.readFileSync(
            path.join(options.directory, compound.refid + '.xml'), 'utf8');
        xmlParser.parseString(doxygen, function(err, data) {
          if (err) {
            log.verbose('warning - parse error for file',
                        path.join(options.directory, compound.refid + '.xml'))
            return;
          }
  ...
  loadIndex : function(options, callback) {
    ...
    fs.readFile(
        path.join(options.directory, 'index.xml'), 'utf8', function(err, data) {
          if (err) {
